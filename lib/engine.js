...
class PoolNoodle extends Component {
	...
	process_request(args, callback) {	
		...
		if (app_stats) app_stats.active++;
		
		var route_stats = this.stats.routes[ route.id ];
		if (route_stats) route_stats.active++;
		
		// rate limiting (app level and route level)
		var rate_limit_err = '';
		
		if (app.max_requests_per_sec && app_stats && (app_stats.sec > app.max_requests_per_sec)) {
			rate_limit_err = 'Exceeded app ' + route.app_id + ' maximum requests per second (' + app_stats.sec + ' > ' + app.max_requests_per_sec + ')';
		}
		else if (app.max_concurrent_requests && app_stats && (app_stats.active > app.max_concurrent_requests)) {
			rate_limit_err = 'Exceeded app ' + route.app_id + ' maximum concurrent requests (' + app_stats.active + ' > ' + app.max_concurrent_requests + ')';
		}
		else if (route.max_requests_per_sec && route_stats && (route_stats.sec > route.max_requests_per_sec)) {
			rate_limit_err = 'Exceeded route ' + route.app_id + '/' + route.type + '/' + route.id + ' maximum requests per second (' + route_stats.sec + ' > ' + route.max_requests_per_sec + ')';
		}
		else if (route.max_concurrent_requests && route_stats && (route_stats.active > route.max_concurrent_requests)) {
			rate_limit_err = 'Exceeded route ' + route.app_id + '/' + route.type + '/' + route.id + ' maximum concurrent requests (' + route_stats.active + ' > ' + route.max_concurrent_requests + ')';
		}
