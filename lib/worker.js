...
module.exports = {
	...
	startup: function(worker, callback) {
		...
		if (this.serverConfig.echo || this.serverConfig.worker_echo) {
			...
			process.stderr.on('error', function() {}); // prevent stderr pipe errors
		...
		async.eachSeries(
			...
			function(err) {
				...
				script.handler( args, function() {
					if (callback_called) {
						self.logError(app_id, "WARNING: Double callback detected in: " + app_id + ": " + file, { url: args.url });
						return;
					}
	...
	shutdown: function(callback) {
		...
		if (this.starting) {
			// delay shutdown if worker is still starting up
			setTimeout( function() { self.shutdown(callback); }, 250 );
			return;
		}
